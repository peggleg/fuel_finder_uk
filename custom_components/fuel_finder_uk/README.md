...
> **Just downloaded all the files?** See `FOLDER_SETUP.md` to organize them into the correct structure. Run `setup.sh` (Mac/Linux) or `setup.bat` (Windows) to set it up automatically.

Gets the cheapest petrol and diesel prices near you, updates every 15 minutes, and puts them in Home Assistant so you can build automations around them. Want to know when diesel drops below 140p? Set up an automation. Need the station address? It's there. Track more than one address (home and work, say) and each gets its own set of sensors. Got a station you already go to out of habit? Tag it as a favourite and see its prices sitting right next to the cheapest one.

Tracks all four fuel types the UK government publishes: E10 and E5 petrol, B7 diesel, and super diesel.
...
Got a station you already go to, out of habit or because it's on your way somewhere? Pick it as a favourite and you get its prices sitting right next to the cheapest ones. No extra maths, no "is it worth it" verdict built in. You look at both and decide for yourself whether the drive is worth it.

Picking one gives you a "Favourite Station" dropdown, plus four more price sensors (E10, E5, diesel, super diesel) for whichever station you chose, in the same shape as the cheapest-price sensors, address and distance included.

The dropdown only lists stations that were near you when you first set the integration up, kept separate from whatever search radius you're using for price comparisons day to day. So if you shrink that radius later, your favourite doesn't disappear or lose its data. If you widen it, the dropdown's choices won't grow to match, since they're still drawn from the original setup radius. If you genuinely move and want a fresh set of options, remove and re-add the integration.

Leave the dropdown on "None" and nothing changes. The four favourite-price sensors just sit there with nothing to show until you pick something.
...
If you've tagged a favourite station, its E10, E5, diesel, and super diesel prices show up as their own sensors too, right alongside the cheapest-price ones.
...
content: |
  ...
  E10: {{ states('sensor.cheapest_petrol_e10_price') }}p at {{ state_attr('sensor.cheapest_petrol_e10_price', 'station_name') }}
  
  E5: {{ states('sensor.cheapest_petrol_e5_price') }}p at {{ state_attr('sensor.cheapest_petrol_e5_price', 'station_name') }}
  
  Diesel: {{ states('sensor.cheapest_diesel_price') }}p at {{ state_attr('sensor.cheapest_diesel_price', 'station_name') }}
  
  {{ state_attr('sensor.cheapest_petrol_e10_price', 'distance') }}mi away
  
  Updated at {{ as_timestamp(states('sensor.last_update')) | timestamp_custom('%H:%M') }}
```

**Get notified when petrol drops below 140p:**

```yaml
automation:
  - alias: "Petrol Price Alert"
    trigger:
      - platform: numeric_state
        entity_id: sensor.cheapest_petrol_e10_price
        below: 140
    action:
      - service: notify.mobile_app_YOUR_PHONE
        data:
          title: "Cheap Petrol!"
          message: "Only {{ states('sensor.cheapest_petrol_e10_price') }}p at {{ state_attr('sensor.cheapest_petrol_e10_price', 'station_name') }}"
```

**Compare your favourite against the cheapest:**

```yaml
type: markdown
content: |
  Your usual: {{ states('sensor.favourite_station_e10_price') }}p
  
  Cheapest nearby: {{ states('sensor.cheapest_petrol_e10_price') }}p at {{ state_attr('sensor.cheapest_petrol_e10_price', 'station_name') }}, {{ state_attr('sensor.cheapest_petrol_e10_price', 'distance') }}mi away
```
...
**No stations showing up:** Try a bigger search radius (10 miles instead of 5). Not every station participates, mostly just the big chains. Make sure your coordinates are right by checking them in Google Maps.
...
**Favourite station sensors show unavailable:** Either you haven't picked one yet (dropdown's on "None"), or the station you picked doesn't sell that particular fuel type. Both are expected, not a bug.

**Prices look old:** Retail stations report prices with a 15-minute delay. Some independent stations barely update their prices. The government data is only as fresh as what retailers submit.

## Privacy and data

This reads public fuel prices from the government's official API. No personal data goes anywhere. Prices aren't stored on disk, just kept in memory for the session. You're not being tracked.

## What won't work

Very remote areas might not have enough stations. Independent petrol stations sometimes don't update their prices. The API data lags about 15 minutes. You can't search beyond a certain radius, but it can be pretty big.
...
Pull requests welcome. This is Apache 2.0 licensed so feel free to fork, modify, and improve it.
